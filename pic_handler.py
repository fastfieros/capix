...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_add(self, params):
        ...
        c = con.cursor()
        c.execute('INSERT INTO config (client, file_title, tag_filter, rating, datesize, titlesize) VALUES (:name, 1, "*", ">0", 60, 40)', {"name":name})
        con.commit()
        c.close()
        con.close()
    ...
    def do_update(self, params):
        ...
        c.execute('UPDATE config SET file_title=:file_title, tag_filter=:tag_filter, rating=:rating, datesize=:datesize, titlesize=:titlesize WHERE client=:name',
    ...
    def do_config(self, params):
            ...
            con = sqlite3.connect(DB_PATH)
            c = con.cursor()
            c.execute('SELECT client, file_title, tag_filter, rating,  datesize, titlesize FROM config')
            rows = c.fetchall()
            ...
            for client in rows:
                ...
                confpage += "<form method='get' action='/config'>"+\
                            ...
                            "Use filename when title is empty: <input type='checkbox' name='file_title' {}/><br>".format(client[1] == 1 and "checked" or "")+\
    ...
    def draw_text(self, jpgimg, title, datesize, titlesize):
            ...
            with Drawing() as draw:  
                ...
                draw.font = SCRIPT_PATH + '/font/PoiretOne-Regular.ttf'
                draw.fill_color = Color('white')
                draw.fill_opacity = 0.9
                draw.fill_width = 8
    ...
    def do_GET(self):
        ...
        c.execute('SELECT file_title, tag_filter, rating, datesize, titlesize FROM config WHERE client=:client',
                  {"client":client})
        row = c.fetchone()
        if not row:
            self.send_response(404, 'Not Found')
        ...
        datesize = row[3]
        titlesize = row[4]
        print("{}/{}/{}/{}/{}".format(file_title,tagfilter,stars,datesize,titlesize))
        ...
        if not row:    
            ...
            with Image(filename=PIC_PATH + path) as jpgimg:
                ...
                self.draw_text(jpgimg, label, datesize, titlesize)
