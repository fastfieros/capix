...
def build_db(cur, basepath):
    ...
    cur.execute('DELETE FROM xtags WHERE rowid IN (SELECT xtags.rowid FROM xtags JOIN pics ON pics.rowid = picid WHERE pics.dirty=1)')
...
def rebuild_db(db_path, basepath):
    ...
    if row:
        ...
        cur.execute('CREATE TABLE pics (path text unique, views int, dirty boolean, stars float, title text, description text, mtime int)')
        ...
        cur.execute('CREATE TABLE config (client text unique, file_title int, tag_filter text, rating text, datesize int, titlesize int)')
        cur.execute('INSERT INTO config VALUES ("default", 1, "*", ">0", 60, 40)')
